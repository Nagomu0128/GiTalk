...
### E2E テスト中に発見・修正した問題
1. **Push で 500 エラー:** `originalCreatedAt` に文字列が渡され `toISOString()` が呼べない → `new Date()` でラップして修正
2. **会話削除 UI がない:** 会話カードにミートボールメニュー（⋯）+ 削除確認ダイアログを追加
...
- **E2E テスト: パス**
  1. 会話作成 → メッセージ送信 ✅
  2. 「📦 保存」→ Push ダイアログ → 新規リポジトリ作成 + Push ✅
  3. リポジトリ一覧にリポジトリ表示 ✅
  4. リポジトリ詳細 → ブランチ一覧 + 会話表示 ✅
  5. 会話カード ⋯ → 削除確認ダイアログ → 削除 → カード消える ✅
