...
## スキップした項目
- **E2E 動作確認:** バックエンド + フロントエンド同時起動でのブラウザテストは未実施 -> 実施済み
...
- **E2E 動作確認: パス**
  1. ダッシュボードから「新しい会話を始める」→ 会話画面に遷移 ✅
  2. ウェルカム画面表示（ノード 0 個、ツリー非表示） ✅
  3. メッセージ送信 → AI 応答がストリーミング表示 ✅
  4. 送信完了後、左側にツリーノードが表示 ✅
  5. 複数メッセージ送信 → ノードが連なって表示 ✅
  6. タイトル自動生成 → ヘッダーのタイトルが更新 ✅

### E2E テスト後の UX 修正
- **ユーザーメッセージ即時表示:** 送信直後にユーザーメッセージをチャット欄に即座に表示。AI応答待ちの間は「🤖 AIが考えています...」ローディングアニメーションを表示。ストリーミング開始で応答テキストに切替
- **手動ノード接続の無効化:** React Flow のデフォルト機能でユーザーがノード間をドラッグで接続できてしまう問題を `nodesConnectable={false}` で無効化。`nodesDraggable={false}` も追加（ELK.js 自動レイアウトとの不整合防止）
- **MiniMap 削除:** ノード数が少ない段階では不要。ユーザーの要望で削除
