...
// 認証必須ミドルウェア
// app.use("/v1/conversations/*", authMiddleware) のように適用
...
// ルートハンドラから認証ユーザーを取得するヘルパー
const getAuthUser = (c: Context) => c.get("authUser");

// ルートハンドラでの使用例（src/routes/conversations.ts）
