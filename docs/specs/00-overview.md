...
- AIチャット基本機能（Gemini API連携、ストリーミング対応）
- ツリー構造の会話管理（ノード、ブランチ）
- Git的操作（branch, switch/checkout, merge, reset, diff）
- GitHub的機能（リポジトリ保存、公開範囲制御（private/public）、選択的push）
- Firebase Authenticationによるユーザー認証（Google / メール・パスワード）
- React Flow + ELK.js によるツリー可視化UI

### 含まない（将来機能）

- cherry-pick、clone（MVP後の優先実装）
- 共同編集（リアルタイム同期） — ただし拡張性は確保
- limited_access（公開範囲の第三の選択肢）
- Explore（公開リポジトリ探索）
- スター（ブックマーク）機能
