...
|--------|------|------|------|-----|
| POST | `/conversations` | 新規会話作成 | 必須 | Yes |
| GET | `/conversations` | 自分の会話一覧取得 | 必須 | Yes |
| GET | `/conversations/:id` | 会話詳細取得 | 必須 | Yes |
| PATCH | `/conversations/:id` | 会話情報更新（タイトル等） | 必須 | Yes |
| DELETE | `/conversations/:id` | 会話削除 | 必須 | Yes |
...
| Method | Path | 説明 | 認証 | MVP |
|--------|------|------|------|-----|
| POST | `/conversations/:id/branches` | ブランチ作成 | 必須 | Yes |
| GET | `/conversations/:id/branches` | ブランチ一覧取得 | 必須 | Yes |
| PATCH | `/conversations/:id/branches/:branchId` | ブランチ更新（名前変更） | 必須 | Yes |
| DELETE | `/conversations/:id/branches/:branchId` | ブランチ削除 | 必須 | Yes |
...
| Method | Path | 説明 | 認証 | MVP |
|--------|------|------|------|-----|
| GET | `/conversations/:id/nodes` | 全ノード取得（ツリー構築用） | 必須 | Yes |
| GET | `/conversations/:id/nodes/:nodeId` | ノード詳細取得 | 必須 | Yes |
| GET | `/conversations/:id/nodes/:nodeId/path` | ノードからルートまでのパス取得 | 必須 | Yes |
...
| Method | Path | 説明 | 認証 | MVP |
|--------|------|------|------|-----|
| POST | `/conversations/:id/chat` | メッセージ送信（SSEストリーミング） | 必須 | Yes |
...
| Method | Path | 説明 | 認証 | MVP |
|--------|------|------|------|-----|
| POST | `/conversations/:id/merge` | ブランチのマージ | 必須 | Yes |
| POST | `/conversations/:id/reset` | ブランチのリセット | 必須 | Yes |
| POST | `/conversations/:id/cherry-pick` | ノードのcherry-pick | 必須 | No |
| GET | `/conversations/:id/diff` | ブランチ間の差分取得 | 必須 | Yes |
...
| Method | Path | 説明 | 認証 | MVP |
|--------|------|------|------|-----|
| POST | `/repositories` | リポジトリ作成 | 必須 | Yes |
| GET | `/repositories` | 自分のリポジトリ一覧 | 必須 | Yes |
| GET | `/repositories/:id` | リポジトリ詳細 | 条件付き | Yes |
| PATCH | `/repositories/:id` | リポジトリ更新 | 必須 | Yes |
| DELETE | `/repositories/:id` | リポジトリ削除 | 必須 | Yes |
| GET | `/repositories/:id/branches` | リポジトリのブランチ一覧 | 条件付き | Yes |
| GET | `/repositories/:id/nodes` | リポジトリの全ノード | 条件付き | Yes |
...
| Method | Path | 説明 | 認証 | MVP |
|--------|------|------|------|-----|
| POST | `/repositories/:id/push` | 会話をリポジトリにpush | 必須 | Yes |
...
| Method | Path | 説明 | 認証 | MVP |
|--------|------|------|------|-----|
| GET | `/explore/repositories` | 公開リポジトリ一覧 | 不要 | No |
